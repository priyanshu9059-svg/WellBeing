...
export function SettingsPage() {
  ...
  const [anon, setAnon] = useState(() => {
    try {
      return localStorage.getItem('wellbeing-support:pref-anonymous') !== 'false';
    } catch {
      return true;
    }
  });
  const [reduced, setReduced] = useState(() => {
    try {
      return localStorage.getItem('wellbeing-support:pref-reduced-sensory') === 'true';
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    document.documentElement.dataset.reducedSensory = reduced ? 'true' : 'false';
  }, [reduced]);
  useEffect(() => {
    let cancelled = false;
    function refreshStoredCount() {
      try {
        setStoredCount(Object.keys(localStorage).filter((k) => k.startsWith('wellbeing-support:')).length);
      } catch {
        setStoredCount(0);
      }
    }
  ...
  async function logOut() {
    try {
      await getServices().authentication.signOut();
    } catch {}
    try {
      signOutLocalUser();
    } catch {}
    ...
    <div className="settings-page">
      <Card className="settings-hero">
        <div>
          ...
          <h2>Manage your profile and records</h2>
          <p>Keep your account details clear, review optional personal information, and download your saved wellbeing records from this browser.</p>
        </div>
        <span className="settings-state"><Check />{isApiEnabled() ? 'API sync available' : 'Local records'}</span>
      </Card>
      <div className="settings-layout">
        <aside className="settings-tabs" aria-label="Settings sections">
          ...
          <button className={section === 'records' ? 'active' : ''} onClick={() => setSection('records')}><Download /><span><b>Download records</b><small>{storedCount} saved items</small></span></button>
        </aside>
        <Card className="settings-panel">
          {section === 'profile' && (
            <section>
              <p className="kicker">Profile edit</p>
              <h2>Edit how your account works</h2>
              <div className="settings-form-grid">
                <div className="field"><span>{t('language')}</span><LanguageSelect id="settings-language" /></div>
                <label className="check"><input type="checkbox" checked={anon} onChange={(e) => { setAnon(e.target.checked); localStorage.setItem('wellbeing-support:pref-anonymous', String(e.target.checked)); }} /><span><b>{t('anonymousMode')}</b><small>{t('anonymousModeCopy')}</small></span></label>
                <label className="check"><input type="checkbox" checked={reduced} onChange={(e) => { setReduced(e.target.checked); localStorage.setItem('wellbeing-support:pref-reduced-sensory', String(e.target.checked)); }} /><span><b>{t('reducedSensory')}</b><small>{t('reducedSensoryCopy')}</small></span></label>
              </div>
              <div className="settings-action-row">
                <Link href="/profile" className="btn btn-primary">Edit profile <ArrowRight /></Link>
                <Link href="/signup" className="btn btn-secondary">Create account</Link>
                <button className="btn btn-danger settings-logout" onClick={logOut}><LogOut />Log out</button>
              </div>
            </section>
          )}
          {section === 'details' && (
            <section>
              ...
              <h2>Review optional details</h2>
              <p className="muted">These fields are optional and can be changed anytime. Share them only when they help your care experience.</p>
              ...
              <div className="settings-action-row">
                <Link href="/profile" className="btn btn-primary">Update personal details <ArrowRight /></Link>
                <Link href="/privacy" className="btn btn-secondary">Privacy center</Link>
              </div>
            </section>
          )}
          {section === 'records' && (
            <section>
              <p className="kicker">Download records</p>
              <h2>Export your wellbeing data</h2>
              <p className="muted">Downloads are JSON files created from the Wellbeing Support records saved in this browser. Server records may depend on backend retention settings.</p>
              <div className="record-download-grid">
                <button onClick={() => downloadRecords('all')}><Download /><b>All records</b><small>Profile, journal, mood, safety plan, consent, and app data.</small></button>
                <button onClick={() => downloadRecords('profile')}><IdCard /><b>Profile records</b><small>Account, personal details, and consent preferences.</small></button>
                <button onClick={() => downloadRecords('activity')}><FileText /><b>Activity records</b><small>Conversation, journal, mood, appointments, and safety plan.</small></button>
              </div>
              {downloaded && <p className="success-box"><Check />{downloaded}</p>}
              <Link href="/privacy" className="text-link">Open full privacy and data controls →</Link>
            </section>
          )}
        </Card>
      </div>
...
export function AboutPage() {
  return (
    <div className="prose-page">
      <Card>
        <p className="kicker">About Wellbeing Support</p>
        <h2>Support designed around choice.</h2>
        <p>Wellbeing Support helps people talk, pause, reflect, or prepare. It uses warm language, small steps, and anonymous access by default.</p>
        <p>It is not a therapist, clinician, emergency dispatcher, diagnostic tool, or substitute for professional care. When the API is configured, data can sync to a secure backend you control.</p>
      </Card>
      <Card>
        <h2>Cultural and language awareness</h2>
        <p>People describe distress differently across languages, families, workplaces, campuses, and communities. The interface offers English, Hindi, and Hinglish choices as a prototype; complete translated content and cultural review remain future work.</p>
      </Card>
    </div>
  );
}
...
export function FaqPage() {
  const qs = [
    ['Is this therapy?', 'No. It is a wellbeing support app with guided tools and optional AI companion replies — not a clinical service.'],
    ['Is my information private?', 'Anonymous mode is default. With the API enabled, data syncs to your backend under consent rules. Local browser storage is also used as a cache and is not described as encrypted.'],
    ['Do I need an account?', 'No for everyday support. Professionals sign in for the care portal. Anonymous sessions still get a secure session token when the API is on.'],
    ['Can this contact emergency services for me?', 'No automatic dispatch. Phone links open only after you click. Notification and counsellor handoff require your consent.'],
    ['Can it diagnose anxiety or depression?', 'No. Summaries and voice or camera signals are explicitly non-diagnostic.'],
    ['Will it work offline?', 'After a successful visit in a supported production browser, the PWA provides a basic offline fallback and locally saved tools.'],
  ];
  return (
    <div className="faq-list">
      {qs.map(([q, a]) => (
        <details key={q}><summary>{q}<CircleHelp /></summary><p>{a}</p></details>
      ))}
    </div>
  );
}
  ...
  return (
    <div className="narrow-page">
      <Card className="empty-state large">
        ...
        <p>Locally saved tools may still be available. Reconnect to load pages that have not been visited before.</p>
        <Link href="/support" className="btn btn-primary">Try support home</Link>
      </Card>
    </div>
  );
}
  ...
  return (
    <div className="narrow-page">
      <Card className="empty-state large">
        ...
        <p>Nothing went wrong with your session. Choose a support tool to continue.</p>
        <Link href="/support" className="btn btn-primary">Return to support</Link>
      </Card>
    </div>
  );
}

export function FeatureFlagsNote() {
  return <>{features.voice && <Camera className="sr-only" />}</>;
}
