...
export function AppShell({children,title,description}:{children:React.ReactNode;title:string;description?:string}){
  ...
  return <div className="app-frame"><PwaRegister/><a className="skip-link" href="#main-content">Skip to content</a>
    <header className="app-header"><Link href="/" className="brand"><span aria-hidden="true">✦</span>{brand.name}</Link><div className="header-tools"><span className="privacy-pill"><Shield size={14}/> Anonymous · local only</span><label className="sr-only" htmlFor="app-language">Language</label><select id="app-language" defaultValue="English"><option>English</option><option>हिन्दी</option><option>Hinglish</option></select><button className="mobile-menu" onClick={()=>setMenu(!menu)} aria-label="Toggle menu">{menu?<X/>:<Menu/>}</button></div></header>
